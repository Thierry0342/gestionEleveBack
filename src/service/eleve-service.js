...
async function create(eleve, options = {}) {
  return Eleve.create(eleve, options);
}
...
async function deleteByPk(id) {
  ...
  await Promise.all([
    ...
    ...((eleve.specialites || []).map(s => s.destroy())), // ✅ nouveau
  ]);
...
async function updateById(id, updatedData, options = {}) {
  ...
  if (!eleve) throw new Error(`Élève avec l'ID ${id} non trouvé`);
  ...
  if (updatedData.famille) {
    ...
    const updateOrCreate = async (Model, data) => {
      ...
      const { id: _ignoredId, ...dataSansId } = data;
      if (exist) {
  ...
  if (updatedData.sports) {
    const sportPayload = {
      eleveId: id,
      Football: false, Basketball: false, Musculation: false,
      Rugby: false, Volley_ball: false, Athletisme: false,
      Tennis: false, ArtsMartiaux: false, Autre: false,
    };
  ...
  if (updatedData.Diplome && typeof updatedData.Diplome === 'object') {
    const diplomePayload = {
      eleveId: id,
      CEPE: false, BEPC: false, BACC_S: false, BACC_L: false,
      BACC_TECHNIQUE: false, Licence: false, MasterOne: false,
      MasterTwo: false, Doctorat: false,
    };
